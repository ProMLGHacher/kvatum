...
export const RegisterPage = () => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            requestRegistrationAction({ email, nickname, password })
                .then(_ => setRequestedPassword(email))
        } catch (error) {
            isAxiosError
    ...
    const handleCompleteRegistration = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            registerAction({ email, verificationCode: code })
                .then(_ => setRequestedPassword(email))
        } catch (error) {
            isAxiosError
            if (isActionError(error)) {
    ...
    if (requestedPassword) {
        return <div className={cls.registerPage}>
            <form className={cls.registerForm} onSubmit={handleCompleteRegistration}>
                <Input type="text" placeholder="Code" autoComplete='one-time-code' value={code} onChange={(e) => setCode(e.target.value)} />
                <Button type="submit" full rounded='sm'>Register</Button>
    ...
    return (
        <div className={cls.registerPage}>
            <form className={cls.registerForm} onSubmit={handleSubmit}>
                ...
                <Input type="text" placeholder="Nickname" autoComplete='nickname' value={nickname} onChange={(e) => setNickname(e.target.value)} />
