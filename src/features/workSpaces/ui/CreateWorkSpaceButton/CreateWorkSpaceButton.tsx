...
export const CreateWorkSpaceButton = () => {
    ...
    const [isOpenCreateWorkSpaceModal, setIsOpenCreateWorkSpaceModal] = useState(false)
    ...
    return (
        <>
            <PopUp isOpened={isOpenCreateWorkSpaceModal} onClose={() => setIsOpenCreateWorkSpaceModal(false)}>
                <div className={cls.createHub}>
                    <h2>Create WorkSpace</h2>
                    <p>Create a new workSpace to start collaborating with your team.</p>
                    <Input value={workSpaceName} style={{  }} onChange={(e) => setWorkSpaceName(e.target.value)} placeholder='Enter workSpace name' />
                    <Button onClick={handleCreateWorkSpace}>Create WorkSpace</Button>
                </div>
            </PopUp>
            <button className={cls.createHubButton} onClick={() => setIsOpenCreateWorkSpaceModal(true)}>
                +
            </button>
        </>
    )
