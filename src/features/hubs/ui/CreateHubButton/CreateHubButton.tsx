...
export const CreateHubButton = () => {
    ...
    return (
        <>
            <PopUp isOpened={isOpenCreateHubModal} onClose={() => setIsOpenCreateHubModal(false)}>
                <div className={cls.createHub}>
                    <h2>Create Hub</h2>
                    <p>Create a new hub to start collaborating with your team.</p>
                    <Input value={hubName} style={{  }} onChange={(e) => setHubName(e.target.value)} placeholder='Enter hub name' />
                    <Button onClick={handleCreateHub}>Create Hub</Button>
                </div>
            </PopUp>
            <button className={cls.createHubButton} onClick={() => setIsOpenCreateHubModal(true)}>
                +
            </button>
        </>
    )
